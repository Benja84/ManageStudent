          ...
          <div class="card-body">
            ...
            <div class="form-group mt-3">
              ...
              <select class="select2 form-select shadow-none" name="professor_id" id="professor">
                ...
                @foreach($course->subject->professors as $prof)
                  <option value="{{ $prof->id }}" @if(old('professor_id',$course->professor_id) == $prof->id) selected @endif>{{ $prof->user->firstname }} {{ $prof->user->lastname }}</option>
                @endforeach
              </select>
            </div>
            <div class="form-group mt-3">
              ...
              <select class="select2 form-select shadow-none" name="room_id">
                ...
                @foreach($rooms as $room)
                  <option value="{{ $room->id }}"  @if(old('room_id',$course->room_id) == $room->id) selected @endif>{{ $room->name }}</option>
                @endforeach
              </select>
            </div>
            <div class="form-group mt-3">
            ...
            <div class="form-group mt-3">
              ...
              <div class="d-flex justify-content-between">
                <div class="col-md-2">
                  <select class="select2 form-select shadow-none col-md-2"  name="weekday">
                    ...
                    @foreach (weekdays() as $weekday => $localeWeekday)
                      <option class="form-control" data-tokens="{{ $localeWeekday }}" @if(old('weekday',$course->weekday) == $weekday) selected @endif
                        value="{{ $weekday }}">{{ $localeWeekday }}</option>
                    @endforeach
                  </select>
                </div>
                <div class="col-md-2">
                  <input class="form-control col-md-2 js-masked-time start_time" value="@if(old('start_time')|| date('H:i', strtotime($course->start_time))) {{old('start_time',date('H:i', strtotime($course->start_time)))}} @endif" type="text" name="start_time" placeholder="Heure de debut du cours (HH:MM)">
                </div>
                <div class="col-md-2">
                  <select class="select2 form-select shadow-none col-md-2" name="duration">
                    ...
                    @for ($duration = 0.5; $duration < 10; $duration+=0.5)
                      <option class="form-control" data-tokens="{{ $duration }} heure(s)"
                        @if(old('duration',$course->duration) == $duration) @php($selected = TRUE) selected @endif
                ...
                <div class="col-md-2">
                  <input class="form-control col-md-2 start_date datepicker" value="{{old('start_date',date('d/m/Y', strtotime($course->date)))}}" type="text" name="start_date" placeholder="Date début de la période (dd/mm/yyyy)">
                </div>
                <div class="col-md-2">
                  <input class="form-control col-md-2 end_date  datepicker" value="{{old('end_date',date('d/m/Y', strtotime($course->date)))}}" type="text" name="end_date" placeholder="Date fin de la période (dd/mm/yyyy)">
                </div>
              </div>
            </div>
          </div>
